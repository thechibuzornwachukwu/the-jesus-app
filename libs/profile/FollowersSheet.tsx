...
import { getFollowers } from '../../lib/follow/actions';
...
export function FollowersSheet({ open, onClose, userId }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    getFollowers(userId).then((data) => { setPeople(data); setLoading(false); });
  }, [open, userId]);

  return (
    <BottomSheet open={open} onClose={onClose} title="Followers">
