...
import { getFollowingList } from '../../lib/profile/actions';
...
export function FollowingSheet({ open, onClose, userId, title = 'Following' }: FollowingSheetProps) {
  ...
  const loadPage = useCallback(async (p: number) => {
    setLoading(true);
    const data = await getFollowingList(userId, p);
    if (p === 0) {
      setUsers(data);
    } else {
